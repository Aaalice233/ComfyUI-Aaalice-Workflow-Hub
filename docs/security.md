...
- 订阅源必须是公开 `github.com/{owner}/{repo}`，资源访问限制在 GitHub 受信任 HTTPS 主机；订阅清单优先通过 GitHub Contents API 读取，公共 API 限流时仅对该只读清单回退到带强制刷新参数的 GitHub Raw 地址，发布和管理操作使用已认证的 GitHub API。
