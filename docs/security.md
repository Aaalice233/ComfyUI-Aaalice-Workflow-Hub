...
- 输入图像逐项校验清单声明、大小和 SHA-256，并写入隔离的 ComfyUI input 子目录。历史版本 LoRA 不进入主包，只有用户主动选择时才从清单中的 GitHub Release URL 下载；目标路径按原引用置于 ComfyUI LoRA 根目录内，并拒绝越界和覆盖不同内容的同名文件。新发布版本不允许声明 LoRA。
- 不执行包、仓库或清单中的任何代码。
...
- 依赖计划默认只读。安装、升级或降级只有在请求明确携带确认后才执行；Git 依赖由 ComfyUI 环境中的 Git 处理，历史 Registry 依赖提交给 ComfyUI-Manager，所有任务按顺序执行并通过统一操作记录进度和日志。
