...
- 订阅源必须是公开 `github.com/{owner}/{repo}`，资源访问限制在 GitHub 受信任 HTTPS 主机；订阅清单从 `raw.githubusercontent.com` 读取，发布和管理操作才使用已认证的 GitHub API。
...
- 输入图像逐项校验清单声明、大小和 SHA-256，并以临时文件和独占链接写入当前用户专属的 ComfyUI input 子目录；输入图像与工作流文件是两个独立的本地提交阶段，下载失败不会覆盖已有不同内容。工作流包不包含模型或 LoRA 文件，新发布版本也不允许声明 LoRA。
- 不执行包、仓库或清单中的任何代码。
