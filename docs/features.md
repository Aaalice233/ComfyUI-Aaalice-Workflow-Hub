...
- 普通点击在 ComfyUI 内使用非全屏面板加载 `/workflow-hub`；`Shift+点击` 打开独立窗口。
- 内置面板和独立页面通过 `BroadcastChannel` 按需读取当前画布，不写浏览器持久存储。
- 工作流中心采用黑灰桌面工作台布局：宽屏使用侧边导航，窄屏自动切换为顶部导航；订阅页集中展示来源、搜索、筛选和目录状态，发布页按来源、仓库、资料和版本分段组织。
