...
- One-click plugin completion: Git dependencies install at their locked commits with `requirements.txt` handled automatically, while legacy Registry dependencies go through ComfyUI-Manager. Tasks are globally serialized and duplicate requests reuse one operation; stale remote refs are refreshed automatically, uncommitted changes and truly local-only commits are preserved on dedicated Git backup refs, and the completed worktree remains clean on a normal remote-tracking branch instead of a detached HEAD. Progress, per-plugin logs, and errors stay in one view, and downloads still work when completion is unavailable.
...
- Sign in through GitHub Device Flow; credentials prefer the system keyring and access tokens refresh automatically before expiry.

### 🌐 General

- Simplified Chinese, Traditional Chinese, or English interface following the ComfyUI locale automatically.
