...
- Settings can disable automatic update checks or configure the native ComfyUI workflow-update notification interval from 1 to 168 hours. Notification checks are independent of the page catalog cache, and network failures do not consume the scheduled check window. Explicit refreshes bypass Raw's short-lived `HEAD` cache so newly published, deleted, or changelog-edited content becomes visible promptly.
