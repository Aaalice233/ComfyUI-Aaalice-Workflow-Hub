...
- 插件依赖生成只读计划；新 Git 依赖由 ComfyUI 环境中的 Git 按锁定 commit 串行补全，历史 Registry 依赖交给 ComfyUI-Manager，并在同一界面展示进度条、插件状态、安装日志和错误结果；补全不可用时仍可正常下载。

### 发布与管理

- 直接读取当前 ComfyUI 画布，按“确认资源、填写信息、确认发布”三个阶段完成发布；自动识别文件名末尾的 `-v{版本}` 或 `_v{版本}` 并填入名称和版本，下载后的不同版本使用带版本号的独立文件。
- 发布时直接扫描当前 `custom_nodes` 下的 Git 插件仓库，由作者取消勾选与工作流无关的插件，并锁定所选仓库的 GitHub 地址和完整 commit；同时处理画布、子图和侧边栏图像控件的随包图片。检测到 LoRA 引用时仅显示警告，不阻止继续发布；LoRA 不会被自动打包或下载。
- 可选上传封面图（不超过 10 MiB），同时作为工作流封面和版本预览。
- 管理页面向有写权限的作者：编辑资料、归档、编辑版本更新日志、删除版本或整个工作流；订阅侧只提供下载与查看。
