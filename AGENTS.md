...
- 当前 `1.2.7` 已完成可用闭环；文档只能把代码和真实验收已经覆盖的能力描述为已实现。
...
- 发布页的插件依赖必须来自当前环境中可识别的插件仓库，不得把未解析的工作流节点类型伪装成插件。当前发布流程直接扫描 `custom_nodes` 下的 Git 工作副本，并让作者取消勾选无关项目后确认发布清单。
- 新发布的插件依赖统一记录 GitHub `source_url` 与完整 40 位 `commit`，版本检测和补全均以 Git commit 为准；不得把 Comfy Registry 版本或 commit SHA 混写成同一个字段。历史清单中的 Registry 依赖保留由 ComfyUI-Manager 补全的兼容路径。
- Git 与 ComfyUI-Manager 依赖安装必须串行执行，并使用同一个异步操作展示进度、每个插件的状态、原始安装日志和稳定错误码；依赖面板与活动详情面板都必须能查看同一条安装记录，不能只返回“已提交”而隐藏执行结果。
...
- 用户可见文案只能存在于对应运行面的词典：内置 Vue 页面使用 `frontend/src/i18n.ts`，ComfyUI 宿主扩展桥接层使用 `web/comfyui/i18n.js`。新增或修改文案时，必须同时补齐简体中文、繁体中文和英文词典项。
- Vue 组件和普通 TypeScript 业务代码中禁止硬编码中文文案，也禁止使用 `locale === "zh" ? ... : ...`、`locale.value` 分支或其它方式内联维护多语言文案；统一通过 `t("key")` 获取。
- `web/comfyui/workflow_hub.js` 同样禁止硬编码中文、多语言拼接和 `isChinese` 文案分支；统一通过宿主词典的 `translateHost()` 获取。HTML `<title>` 使用不需翻译的产品专名。
- 包含名称、数量、版本等动态内容的文案必须使用词典命名参数，例如 `t("installProgress", { done, total })`；不得在组件中按语言分别拼接句子。
- 专有名词、协议字段、文件名、版本号、单位和后端返回的原始诊断错误可以按原值显示；其周围的说明、标签和操作文案仍必须进入词典。
- 后端新增的可操作用户错误不得拼接本地化提示；应返回稳定的 `error_code` 和 `error_params`，由前端通过 `frontend/src/i18n.ts` 映射并插值。异步操作也必须传递同一错误码，不能把本地化文案写入日志协议。
- Git/Manager 安装日志是原始诊断信息，可以保留命令输出、仓库地址、commit 和 HTTP 状态等原值；面向用户的阶段、状态、按钮、总结和重试提示必须通过简体中文、繁体中文和英文词典呈现。
- 只有第三方原始错误、文件名和调试诊断信息可以原样展示；如果错误需要用户采取行动，行动说明、按钮和数量等动态内容必须进入三种语言词典。
