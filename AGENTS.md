...
- 当前 `1.2.2` 已完成可用闭环；文档只能把代码和真实验收已经覆盖的能力描述为已实现。
...
- 包含名称、数量、版本等动态内容的文案必须使用词典命名参数，例如 `t("installProgress", { done, total })`；不得在组件中分别拼接中英文句子。
...
- Git/Manager 安装日志是原始诊断信息，可以保留命令输出、仓库地址、commit 和 HTTP 状态等原值；面向用户的阶段、状态、按钮、总结和重试提示必须通过中英文词典呈现。
